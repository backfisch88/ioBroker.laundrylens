...
class WashdataAdapter extends utils.Adapter {
      ...
      const manager = new WashDataManager(
        ...
        },
        {
          onStateChange: (state, status) =>
            this._onManagerState(deviceCfg.deviceId, state, status),
          ...
          onCycleFinished: (cycle) =>
            this._onCycleFinished(deviceCfg.deviceId, cycle),
        },
      );
          ...
          if (currentWatts >= (deviceCfg.powerThreshold || 10)) {
            ...
            if (rc && rc.startTime) {
              const profile = manager.profileStore.getProfile(
                manager.currentProgram && manager.currentProgram.id,
              );
              if (profile && profile.durationMs) {
                ...
                await this.setStateAsync(
                  `${deviceCfg.deviceId}.confidence`,
                  Math.round(manager.confidence * 100),
                  true,
                );
                await this.setStateAsync(
                  `${deviceCfg.deviceId}.timeRemaining`,
                  Math.round(remaining / 1000),
                  true,
                );
                await this.setStateAsync(
                  `${deviceCfg.deviceId}.cycleProgress`,
                  progressPct,
                  true,
                );
              }
  ...
  async onUnload(callback) {
    try {
      for (const mgr of Object.values(this.managers)) {
        await mgr.stop();
      }
    } catch (_e) {
      /* ignore */
  ...
  onStateChange(id, state) {
    ...
    const watts =
      typeof state.val === "number" ? state.val : parseFloat(state.val);
    if (isNaN(watts)) {
      return;
    }
  ...
  _handleProgramOverride(deviceId, mgr, val) {
    ...
    if (!val || val === "auto" || val === "") {
      ...
      if (profile) {
        ...
        const avgDuration =
          profile.durationHistory && profile.durationHistory.length > 0
  ...
  async onMessage(obj) {
    ...
    const respond = (r) =>
      obj.callback && this.sendTo(obj.from, obj.command, r, obj.callback);
    ...
    try {
      switch (obj.command) {
        ...
        case "createProfile": {
          ...
          const pid = mgr.profileStore.createManualProfile(
            name,
            (durationMin || 60) * 60_000,
        ...
        case "confirmCycle": {
          ...
          if (cycle.profileId) {
            mgr.profileStore.learnFromCycle(
              cycle.profileId,
              [],
              cycle.durationMs,
            );
        ...
        case "trimTrace": {
          ...
          if (!result) {
            return respond({ error: "Trim fehlgeschlagen" });
          }
          const cycle = mgr.getCycleHistory().find((c) => c.id === cycleId);
          if (cycle) {
        ...
        case "splitTrace": {
          ...
          if (!result) {
            return respond({ error: "Split fehlgeschlagen" });
          }
          ...
          if (idx >= 0) {
            ...
            history.splice(
              ...
              1,
              {
                ...
                id: result.id1,
                endTime: splitTs,
                durationMs: splitTs - orig.startTime,
                hasTrace: true,
              },
              {
                ...orig,
                id: result.id2,
                startTime: splitTs,
                durationMs: orig.endTime - splitTs,
                hasTrace: true,
              },
            );
          }
        ...
        case "clearAntiKnitter": {
          ...
          const akClearDevCfg = this._getDeviceConfig().find(
            (d) => d.deviceId === obj.message.deviceId,
          );
        ...
        case "createProfileFromTrace": {
          ...
          const rawTrace = trace.points.map((p) => ({
            ts: p.ts,
            watts: p.watts,
          }));
          const pid = mgr.profileStore.createProfile(
            name || "Neues Profil",
            rawTrace,
            devCfg ? devCfg.deviceType : "washing_machine",
          );
        ...
        case "importConfig": {
          ...
          for (const p of data.profiles) {
            mgr.profileStore.profiles[p.id] = p;
          }
          await mgr.profileStore.save();
        ...
        case "sendNotification": {
          ...
          if (!notifAdapter || !message) {
            return respond({ error: "adapter und message erforderlich" });
          }
          try {
            const payload = this._buildNotifyPayload(
              notifAdapter,
              target,
              message,
              "LaundryLens Test",
        ...
        case "getTelegramUsers": {
          ...
          try {
            ...
            if (userState && userState.val) {
              const parsed =
                typeof userState.val === "string"
                  ? JSON.parse(userState.val)
                  : userState.val;
              users = Object.entries(parsed).map(([id, u]) => ({
                id,
                name: u.firstName || u.userName || String(id),
              }));
            }
        ...
        case "getNotifyConfig": {
          ...
          try {
            const raw = await this.readFileAsync(
              `laundrylens.${this.instance}.files`,
              `notify_${deviceId}.json`,
            );
            respond({
              ok: true,
              config: raw && raw.file ? JSON.parse(raw.file) : null,
            });
          } catch {
  ...
  async _onCycleFinished(deviceId, cycle) {
    ...
    if (
      mgrPh &&
      mgrPh._phaseHistory &&
      mgrPh._phaseHistory.length > 0 &&
      !(cycle.phaseHistory && cycle.phaseHistory.length > 1)
    ) {
    ...
    this.setState(
      `${deviceId}.lastCycleDuration`,
      Math.round((cycle.durationMs || 0) / 60000),
      true,
    );
    this.setState(
      `${deviceId}.lastCycleEnergy`,
  ...
  async _sendUpdateNotification(
    deviceId,
    checkThreshold,
    remainingSec = 0,
    progressPct = 0,
  ) {
    const devCfgU = this._getDeviceConfig().find(
      (d) => d.deviceId === deviceId,
    );
    ...
    try {
      ...
      if (!cfg || !cfg.adapter || !cfg.updateEnabled) {
        return;
      }
      ...
      if (mgr.cycleStartTime) {
        const activeProf = mgr.currentProgram
          ...
          : mgr._bestCandidate && mgr._bestCandidate.confidence >= 0.5
            ? mgr.profileStore.getProfile(mgr._bestCandidate.id)
            : null;
        if (activeProf && activeProf.durationMs) {
          const elapsed = now2 - mgr.cycleStartTime;
          progressPct = Math.min(
            99,
            Math.round((elapsed / activeProf.durationMs) * 100),
          );
        }
      }
      ...
      if (checkThreshold) {
        ...
        if (state.lastSentAt && now2 - state.lastSentAt < MIN_MS) {
          ...
          if (progressPct >= nearEndPct && state.lastFinishTime) {
            ...
            if (!(diffMin >= nearEndDiffMin && diffPct >= nearEndDiffPct)) {
              return;
            }
          } else {
            return;
        ...
        if (state.lastFinishTime) {
          ...
          const decayedThreshold = Math.max(
            baseThreshold * 0.6,
            baseThreshold - decaySteps * 2,
          );
          const threshold =
            progressPct >= nearEndPct ? nearEndDiffMin : decayedThreshold;
      ...
      const applyTemplate = (tpl) => {
        ...
        return result.replace(/\{(\w+)\}/g, function (m, k) {
          return vars[k] !== undefined ? vars[k] : m;
        });
      };
  ...
  async _sendNotification(deviceId, cycle, event = "done") {
    const devCfgN = this._getDeviceConfig().find(
      (d) => d.deviceId === deviceId,
    );
    ...
    try {
      ...
      if (cfg.target) {
        await this.sendToAsync(
          cfg.adapter,
          this._buildNotifyPayload(cfg.adapter, cfg.target, msg, notifySubject),
        );
      } else {
        await this.sendToAsync(
          cfg.adapter,
          this._buildNotifyPayload(cfg.adapter, null, msg, notifySubject),
        );
      }
      this.log.info(
  ...
  _mgr(msg) {
    ...
    if (deviceId) {
      return this.managers[deviceId] || null;
    }
