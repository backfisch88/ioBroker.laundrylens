...
class TraceStore {
   ...
   *
   * @param {string} cycleId   – ID des Zyklus, dem diese Trace zugeordnet wird
   * @param {Array<{ts, watts}>} rawTrace   – vom CycleDetector
   * @param {number} startTime  – Zyklus-Startzeit (ms, epoch)
   * @param {number} endTime    – Zyklus-Endzeit (ms, epoch)
   */
   ...
   *
   * @param {string} cycleId  – ID des gesuchten Zyklus
   * @returns {{ points: [{ts, watts}], startTime, endTime } | null}  – Trace oder null falls nicht gefunden
   */
  getTrace(cycleId) {
   ...
   *
   * @param {string} cycleId      – ID des zu trimmenden Zyklus
   ...
   *
   * @param {string} cycleId  – ID des zu teilenden Zyklus
   * @param {number} splitTs   – Unix ms
   * @returns {{ part1: object, part2: object }}  – die beiden Teil-Traces
   */
  ...
  /** @param {string} cycleId  – ID des zu löschenden Zyklus */
  deleteTrace(cycleId) {
    delete this.traces[cycleId];
  }

  /**
   * @param {string} cycleId  – zu prüfender Zyklus
   * @returns {boolean}  – true falls eine Trace für diesen Zyklus existiert
   */
  hasTrace(cycleId) {
  ...
  /**
   * Reduziert die Anzahl der Punkte auf `threshold`, ohne die visuelle Form
   * der Kurve wesentlich zu verändern (LTTB-Algorithmus).
   *
   * @param {Array<{t:number, w:number}>} data  – Rohdaten-Punkte
   * @param {number} threshold  – Ziel-Punktzahl
   * @returns {Array<{t:number, w:number}>}  – downgesampelte Punkte
   */
