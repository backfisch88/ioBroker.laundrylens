...
class ProfileStore {
      ...
      if (raw && raw.file) {
        ...
        if (migrated > 0) {
          this.adapter.log.info(
            `[ProfileStore] ${migrated} Profile werden beim nächsten Zyklus neu berechnet (RESAMPLE_LEN geändert)`,
          );
        }
        this.adapter.log.info(
  ...
  async save() {
    try {
      await this.adapter.writeFileAsync(
        ...
        `profiles_${this.deviceId}.json`,
        JSON.stringify(
          { profiles: this.profiles, antiKnitter: this._antiKnitter || null },
          null,
          2,
        ),
      );
  ...
  /**
   * Zählt zusammenhängende Heizblöcke in einer Trace (z.B. für die
   * Spülmaschinen-Phasenzuordnung).
   *
   * @param {Array<{ts:number, watts:number}>} trace  – zu analysierende Leistungskurve
   * @returns {{segments:number, maxHeatDurS:number}}  – Anzahl Heizblöcke und längster gefundener Block (s)
   */
  ...
  /**
   * Erstellt ein neues Profil aus einer echten aufgezeichneten Leistungskurve.
   *
   * @param {string} name  – Anzeigename des Programms
   * @param {Array<{ts:number, watts:number}>} trace  – aufgezeichnete Leistungskurve
   * @param {string} [deviceType]  – Gerätetyp (z.B. 'washing_machine', 'dryer', 'dishwasher')
   * @returns {string}  – ID des neu erstellten Profils
   */
   ...
   *
   * @param {string} name  – Anzeigename des Programms
   * @param {number} durationMs  – erwartete Laufzeit in Millisekunden
   * @param {string} [deviceType]  – Gerätetyp (z.B. 'washing_machine', 'dryer', 'dishwasher')
   * @returns {string}  – ID des neu erstellten Profils
   */
  ...
  /**
   * @param {string} id  – zu löschendes Profil
   * @returns {boolean}  – true falls ein Profil gelöscht wurde
   */
  deleteProfile(id) {
    if (!this.profiles[id]) {
      return false;
  ...
  /**
   * @param {string} deviceType  – Gerätetyp
   * @returns {number}  – gerätetyp-spezifische Konfidenz-Schwelle (0–1)
   */
  _confidenceThresholdFor(deviceType) {
    ...
    if (dt === "washing_machine" || dt === "washer" || dt === "waschmaschine") {
      return Math.min(0.95, MIN_CONFIDENCE + 0.1);
    }
   ...
   * @returns {object|null}  – Profil oder null falls nicht gefunden
   */
  getProfile(id) {
   ...
   *
   * @param {Array<{ts:number, watts:number}>} trace  – aktueller Zyklus
   * @param {number} [toleranceFactor]  – erlaubte Abweichung der Dauer (0.2 = ±20%)
   * @returns {{profileId, name, confidence}|null}  – bestes gefundenes Profil, egal wie niedrig die Konfidenz
   */
  getBestCandidate(trace, toleranceFactor = 0.2) {
    ...
    for (const profile of profiles) {
      ...
      if (profile.durationMs > 0 && currentDurationMs > 0) {
        if (
          currentDurationMs / profile.durationMs >
          1 + toleranceFactor + 0.3
        ) {
          continue;
        }
      }
   ...
   * @returns {number}  – Konfidenz-Score zwischen 0 und 1
   */
  ...
  /**
   * Vergleicht laufende Kurve mit allen Profilen (3-Stage Pipeline: Fast
   * Reject → Pearson-Korrelation → DTW-Tie-Break).
   *
   * @param {Array<{ts:number, watts:number}>} trace  – aktueller Zyklus
   * @param {number} [toleranceFactor]  – erlaubte Abweichung der Dauer (0.2 = ±20%)
   * @returns {{profileId, name, confidence, stage}|null}  – bestätigtes Match oder null
   */
      ...
      if (profile.heatPattern && currentDurationMs > 10 * 60000) {
        ...
        if (
          ...
          this.adapter.log.debug(
            `[ProfileStore] Heat-Reject: ${profile.name} (Block ${Math.round(ph.maxHeatDurS / 60)}min vs ${currentHeat.segments} kurze Peaks)`,
          );
          continue;
        }
        ...
        if (
          ...
          this.adapter.log.debug(
            `[ProfileStore] Heat-Reject: ${profile.name} (${ph.segments} kurze Peaks vs Block ${Math.round(currentHeat.maxHeatDurS / 60)}min)`,
          );
          continue;
        }
      }
   ...
   *
   * @param {string} profileId  – zu aktualisierendes Profil
   * @param {Array<{ts,watts}>} trace  – Leistungskurve des bestätigten Zyklus
   * @param {number} confirmedDurationMs  – tatsächliche Laufzeit des Zyklus
   */
  ...
  /** Plant die erste automatische Wartung auf die nächste Mitternacht, danach täglich. */
  ...
  /** Entfernt leere, nie genutzte manuelle Profile, die älter als 7 Tage sind. */
  _runMaintenance() {
    let removed = 0;
    ...
    if (removed > 0) {
      this.adapter.log.info(
        `[ProfileStore] Maintenance: ${removed} leere Profile entfernt`,
      );
      this.save();
    }
  ...
  /**
   * @param {object} root0  – Referenzwerte aus einem gewählten Zyklus
   * @param {number} root0.maxWatts  – 85. Perzentil der Leistungswerte
   * @param {number} root0.durationMs  – Dauer des Referenzzyklus
   */
  ...
  /** @returns {object|null} – gespeichertes Anti-Knitter-Referenzmuster oder null */
  getAntiKnitter() {
    return this._antiKnitter || null;
  }

  /** Löscht das gespeicherte Anti-Knitter-Referenzmuster. */
  async clearAntiKnitter() {
    this._antiKnitter = null;
    await this.save();
  }

  /** @param {number} pct  – neue Erkennungs-Schwelle in Prozent (0–100) */
  setMatchThreshold(pct) {
    MIN_CONFIDENCE = Math.max(0.1, Math.min(0.95, pct / 100));
    this.adapter.log.debug(
      `[ProfileStore] matchThreshold gesetzt: ${Math.round(MIN_CONFIDENCE * 100)}%`,
    );
  }

  /** @returns {number} – aktuell konfigurierte Erkennungs-Schwelle (0–1) */
  getMatchThreshold() {
    return MIN_CONFIDENCE;
  }
}
