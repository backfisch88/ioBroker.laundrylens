...
class ProfileStore {
   ...
   * @param {number[]} currentWatts  – current power values
   * @param {number} currentDurationMs  – elapsed runtime of the cycle
   ...
   * @param {Array<{phase:string,tMs:number}>} [phaseHistory]  – this cycle's
   *   phase transitions (offsets in ms from cycle start), if known - used to
   *   learn per-phase typical durations for phase-based remaining-time
   *   estimation. Optional and backward-compatible: omitted or empty simply
   *   skips the per-phase learning below.
   */
  learnFromCycle(profileId, trace, confirmedDurationMs, phaseHistory) {
    ...
    // Coefficient of variation (stdDev / mean) of past durations - how much
    // this program's real duration actually swings cycle to cycle (e.g. a
    // dryer's drying time depends heavily on load size/dampness, so it
    // tends to have a much higher CV than a fixed-temperature wash cycle).
    // Used by _updateTimeEstimate() to lean more on the energy-based
    // estimate (a live signal) instead of the historical time average (a
    // single number that represents this specific, possibly quite
    // different, run poorly) when a profile is known to vary a lot.
    // Needs at least 3 samples to be a meaningful estimate of spread.
    if (profile.durationHistory.length >= 3) {
      const n = profile.durationHistory.length;
      const mean = profile.durationMs;
      const variance =
        profile.durationHistory.reduce((s, v) => s + (v - mean) ** 2, 0) / n;
      profile.durationCV = mean > 0 ? Math.sqrt(variance) / mean : 0;
    }

    // ── Per-phase duration learning ──────────────────────────────
    // Learns how long each individual phase (heating, washing, spinning,
    // dryer_drying, cooling, ...) typically takes for THIS profile, rather
    // than only the cycle's total duration. This lets _updateTimeEstimate()
    // estimate remaining time as "time left in the current phase, plus the
    // typical duration of every phase that historically comes after it" -
    // far more accurate near the end of a cycle than a single whole-cycle
    // average, especially for phases (like a dryer's main drying phase)
    // whose position relative to total cycle duration varies a lot.
    if (phaseHistory && phaseHistory.length > 0) {
      // Convert transition points into {phase, durationMs} segments; a
      // phase appearing more than once in one cycle (e.g. "soaking" between
      // two heating blocks) has its segments summed, since remaining-time
      // estimation cares about "how long this phase typically takes in
      // total this cycle", not any one specific occurrence.
      const segmentDurationsByPhase = {};
      const sequence = [];
      for (let i = 0; i < phaseHistory.length; i++) {
        const { phase, tMs } = phaseHistory[i];
        const endTMs =
          i + 1 < phaseHistory.length
            ? phaseHistory[i + 1].tMs
            : confirmedDurationMs;
        const segMs = Math.max(0, endTMs - tMs);
        if (segMs <= 0) {
          continue;
        }
        if (!(phase in segmentDurationsByPhase)) {
          segmentDurationsByPhase[phase] = 0;
          sequence.push(phase);
        }
        segmentDurationsByPhase[phase] += segMs;
      }

      if (sequence.length > 0) {
        profile.phaseStats = profile.phaseStats || {
          sequence: [],
          durations: {},
          durationMs: {},
        };
        // The physical process order is assumed stable for a given
        // profile - each learn simply adopts the most recent cycle's
        // observed order rather than merging orderings across cycles.
        profile.phaseStats.sequence = sequence;
        for (const [phase, segMs] of Object.entries(segmentDurationsByPhase)) {
          profile.phaseStats.durations[phase] =
            profile.phaseStats.durations[phase] || [];
          profile.phaseStats.durations[phase].push(segMs);
          if (profile.phaseStats.durations[phase].length > 20) {
            profile.phaseStats.durations[phase].shift();
          }
          const hist = profile.phaseStats.durations[phase];
          profile.phaseStats.durationMs[phase] =
