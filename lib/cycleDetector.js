 ...
 *
 * v0.4 changes:
 *   - Power trace stored with timestamps (for curve matching)
 *   - Watchdog against stuck cycles
 *   - End-spike protection (dishwasher drain pump discharge)
 *   - Zombie protection (hard limit at 200% of expected duration)
 */
...
const DEFAULT_CONFIG = {
  ...
  offDelay: 300, // s
  startOffDelay: 120, // s – minimum time below threshold in STARTING before falling back to OFF
  minOffGap: 120, // s
  suspiciousWindow: 1200, // s (20 min ghost protection)
  pauseDelay: 60, // s
  endPhaseOffDelay: 60, // s – shortened offDelay during the end phase
  endPhasePowerRatio: 0.2, // < 20% of max watts = end phase
  endPhaseProgressMin: 0.7, // > 70% of expected duration = end phase
  watchdogMs: 2 * 60 * 60 * 1000, // 2h max cycle duration
  zombieFactor: 2.0, // 200% of profile duration = zombie kill
  traceResolutionMs: 10000, // trace point every 10s
};
...
class CycleDetector {
   ...
   * @param {(function(string, string): void)|null} onStateChange  – callback(newState, prevState) fired on every state transition
   * @param {object|null} [adapter]  – optional ioBroker adapter instance. When provided, adapter.setTimeout()/adapter.clearTimeout() are used for the watchdog timer (so it gets cleaned up automatically on adapter unload) and adapter.log is used for diagnostic logging. Falls back to the native global timers when omitted (e.g. in unit tests).
   */
  constructor(config = {}, onStateChange = null, adapter = null) {
    ...
    this._lastTracePoint = 0; // last stored trace point
    ...
    // For zombie protection: expected profile duration
  ...
  processReading(watts, timestamp) {
    ...
    // Accumulate energy (Wh, trapezoidal)
    ...
    // Store power trace at the configured resolution
    if (this.state !== STATES.OFF) {
      ...
      // Track max watts for adaptive offDelay
  ...
  /** Resets the detector after a completed cycle */
  reset() {
    this._stopWatchdog();
    this.state = STATES.OFF;
    this.powerTrace = [];
  ...
  /**
   * Sets the expected profile duration used for zombie protection
   *
   * @param {number} ms  – expected cycle duration in milliseconds
   */
   ...
   */
  restoreTrace(points) {
    // Restore trace from saved state
    // points: [{ts, watts}]
  ...
  _handleOff(watts, now) {
    ...
    // Suppress ghost cycles - only if the last cycle didn't end cleanly
  ...
  _handleStarting(watts, now) {
    if (watts < this.cfg.powerThreshold) {
      // Only fall back to OFF after startOffDelay – prevents flapping
  ...
  _handleEnding(watts, now) {
    // End-spike protection (e.g. dishwasher drain pump discharge)
    if (watts >= this.cfg.powerThreshold) {
      this._setState(STATES.RUNNING);
      return;
    }
    if (this.lastAboveThreshold !== null) {
      ...
      if (offSec >= this.cfg.minOffGap) {
        this.lastCycleEndTime = now;
        this.lastCycleCompleted = true; // Ended cleanly → ghost protection off
        this._stopWatchdog();
        this._setState(STATES.OFF);
      }
    }
  ...
  _startWatchdog() {
    this._stopWatchdog();
    const armTimeout = this.adapter
      ? this.adapter.setTimeout.bind(this.adapter)
      : setTimeout;
    this._watchdogTimer = armTimeout(() => {
      if (this.state !== STATES.OFF) {
  ...
  _stopWatchdog() {
    if (this._watchdogTimer) {
      const clear = this.adapter
        ? this.adapter.clearTimeout.bind(this.adapter)
        : clearTimeout;
      clear(this._watchdogTimer);
      this._watchdogTimer = null;
    }
  }
  ...
  forceEnd(ts) {
    ...
    this.lastCycleEndTime = now;
    this.lastCycleCompleted = true; // Mark as cleanly ended
    this.state = STATES.OFF; // Go straight to OFF without callback
    this.powerTrace = [];
    this.accumulatedEnergy = 0;
    this.belowStartThreshold = null;
    this._stopWatchdog();
