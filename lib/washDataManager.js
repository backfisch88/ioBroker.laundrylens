...
class WashDataManager {
  ...
  /** @returns {string} – Anzeigename des Geräts (Konfigurationsname oder deviceId als Fallback) */
  get _name() {
    return this.config.name || this.config.deviceId;
  }

  /** Lädt Profile, Traces und gespeicherten Zustand und startet die Überwachung. */
  async start() {
    ...
    if (ak) {
      ...
      this.adapter.log.info(
        `${this._name}: Anti-Knitter geladen: ${Math.round(ak.durationMs / 60000)} min, max ${Math.round(ak.maxWatts)}W`,
      );
    }
    ...
    this.adapter.log.info(
      `${this._name} [${this.config.deviceType || "Gerät"}] gestartet`,
    );
  }

  /** Speichert den aktuellen Zustand und stoppt alle laufenden Timer sauber. */
  async stop() {
  ...
  /** Persistiert den laufenden Zyklus-Zustand (für Wiederherstellung nach Neustart). */
  async _saveState() {
    try {
      const liveTrace =
  ...
  /** Lädt gespeicherte Zyklus-Historie und laufenden Zyklus-Zustand aus dem letzten Neustart. */
  async _loadState() {
    try {
  ...
  /** Startet die Überwachung auf "hängende" Leistungswerte (Sensor liefert seit langem keinen neuen Wert). */
    ...
    this._stuckTimer = setInterval(() => {
      ...
      if (stuckMs > STUCK_POWER_MS && this._lastPowerVal > 0) {
        this.adapter.log.warn(
          `${this._name} Stuck-Power: ${this._lastPowerVal}W seit ${Math.round(stuckMs / 60000)} min`,
        );
  ...
  /** Stoppt die Stuck-Power-Überwachung. */
  ...
  /**
   * @param {number} watts  – aktueller Leistungswert
   * @param {number} ts     – Zeitstempel der Messung
   */
  ...
  /**
   * Zentraler Einstiegspunkt: verarbeitet einen neuen Leistungswert,
   * inklusive Anti-Knitter-Sperrlogik für Trockner und Weiterleitung an
   * den CycleDetector.
   *
   * @param {number} watts  – aktueller Leistungswert in Watt
   * @param {number} [timestamp]  – Zeitstempel der Messung (Default: jetzt)
   */
  processPowerReading(watts, timestamp) {
    ...
    // WICHTIG zur Polarität: Checkbox-Label ist "Anti-Knitter ignorieren".
    // AN (true)  = Anti-Knitter-Phase wird ignoriert → einfaches Sofort-Ende,
    //              KEINE Schutzlogik (antiKnitterEnabled = false)
    // AUS (false) = Anti-Knitter-Phase wird NICHT ignoriert → Schutzlogik soll
    //              aktiv versuchen, den echten Zyklus zu erkennen
    //              (antiKnitterEnabled = true, sofern ein Muster existiert)
    ...
    if (!antiKnitterActive && this._dryerLockUntil) {
      // Funktion wurde deaktiviert, Gerätetyp geändert, oder Referenzmuster
      // wurde gelöscht – alte Sperre in jedem Fall verwerfen
      this.adapter.log.debug(
        `${this._name}: Anti-Knitter inaktiv (deaktiviert oder kein Muster) – bestehende Sperrzeit verworfen`,
      );
      this._dryerLockUntil = null;
      this._dryerDropTriggered = false;
      this._dryerHighStart = null;
    ...
    if (antiKnitterActive && this._dryerLockUntil) {
      if (ts >= this._dryerLockUntil) {
        ...
        this.adapter.log.debug(
          `${this._name}: Anti-Knitter Sperrzeit abgelaufen`,
        );
        this._dryerLockUntil = null;
        this._dryerDropTriggered = false;
        this._dryerHighStart = null;
        ...
        if (watts > realThreshold) {
          if (!this._dryerHighStart) {
            ...
            this.adapter.log.debug(
              `${this._name}: Möglicher neuer Zyklus – ${watts.toFixed(0)}W > ${realThreshold}W, warte 30s...`,
            );
          }
          ...
          if (highDurS >= 30) {
            ...
            this.adapter.log.info(
              `${this._name}: Echter neuer Zyklus nach Piepton erkannt (>${realThreshold}W für 30s) – Sperrzeit aufgehoben`,
            );
            this._dryerLockUntil = null;
            this._dryerDropTriggered = false;
            this._dryerHighStart = null;
          ...
          if (this._dryerHighStart) {
            this.adapter.log.debug(
              `${this._name}: Anti-Knitter – ${watts.toFixed(0)}W unter Schwelle, High-Timer zurückgesetzt`,
            );
          }
          this._dryerHighStart = null;
          this.adapter.log.debug(
            `${this._name}: Anti-Knitter Spike ignoriert (${watts.toFixed(0)}W < ${realThreshold}W Schwelle)`,
          );
          return;
        }
      }
    ...
    // WICHTIG: Diese ganze Sofort-Beenden-Logik ergibt nur Sinn, wenn ein
    // gelerntes Anti-Knitter-Muster existiert, gegen das die Sperrzeit
    // bemessen wird. Ohne Muster (z.B. gerade gelöscht) gibt es nichts
    // sinnvoll zu "überspringen" – dann muss die normale, langsamere
    // Ending-Erkennung (offDelay-basiert) greifen, statt bei jedem
    // beliebigen Leistungsabfall sofort abzubrechen.
      ...
      if (
        ...
        if (trace.length >= 3) {
          ...
          if (prevAvg > 400 && watts < 5) {
            ...
            this.adapter.log.info(
              `${this._name}: Trockner-Abfall erkannt (${prevAvg.toFixed(0)}W → ${watts}W) – Zyklus beendet in 45s, Sperrzeit ${Math.round(lockMs / 60000)} min`,
            );
  ...
  /**
   * Wird vom CycleDetector bei jedem Zustandswechsel aufgerufen. Setzt
   * Matching-Zustand bei Zyklusstart zurück und löst Zyklusabschluss aus.
   *
   * @param {string} newState  – neuer State (STATES.*)
   * @param {object} eventData  – Snapshot des Zyklus-Zustands vom Detector
   */
  ...
  /** Führt einen Matching-Durchlauf gegen alle gespeicherten Profile aus (übersprungen bei aktivem manuellen Override). */
  _runMatching() {
    ...
    if (profiles.length > 0 && currentDurationMs > 0) {
      ...
      let minWaitMs = minProfileDurMs * 0.3; // 30% der kürzesten Programmdauer
      ...
      if (currentDurationMs < minWaitMs) {
        this.adapter.log.debug(
          `${this._name}: Matching – warte noch (${Math.round(currentDurationMs / 60000)}min < ${Math.round(minWaitMs / 60000)}min Minimum)`,
        );
    ...
    const bestInfo = best
      ...
      : "–";
    this.adapter.log.debug(
      `${this._name}: Matching – Trace ${trace.length} Punkte, Ergebnis: ${result ? `${result.name} (${Math.round((result.confidence || 0) * 100)}%)` : "kein Match"} | bestCandidate: ${bestInfo} | Schwelle: ${Math.round(this.profileStore.getMatchThreshold() * 100)}%`,
    );
    ...
    if (
      ...
      if (this._instantConfirmPending.count >= INSTANT_CONFIRM_ROUNDS) {
        this.adapter.log.info(
          `${this._name}: Sofortübernahme – "${best.name}" (${Math.round(best.confidence * 100)}%, ${INSTANT_CONFIRM_ROUNDS}x stabil ≥${Math.round(INSTANT_CONFIRM_CONFIDENCE * 100)}%)`,
        );
      ...
      if (!this.currentProgram || result.profileId !== this.currentProgram.id) {
        ...
        if (this._programLocked && this.currentProgram) {
          ...
          if (this._pendingMatch.count >= OVERRIDE_PERSIST) {
            this.adapter.log.info(
              `${this._name}: Override – wechsle gelocktes Programm zu "${result.name}" (${Math.round(result.confidence * 100)}%, ${OVERRIDE_PERSIST}x stabil)`,
            );
  ...
  /**
   * Übernimmt ein erkanntes Programm als aktuell aktives Programm.
   *
   * @param {string} profileId  – ID des erkannten Profils
   * @param {string} name       – Anzeigename des Programms
   * @param {number} confidence  – Erkennungs-Konfidenz (0–1)
   */
  _setProgram(profileId, name, confidence) {
    ...
    this.adapter.log.debug(
      `${this._name}: Programm erkannt: "${name}" (${(confidence * 100).toFixed(1)}%)`,
    );
    if (typeof this.callbacks.onProgramChange === "function") {
  ...
  /** Setzt den Matching-Zustand zurück auf "detecting..." (z.B. nach zu vielen Fehltreffern). */
  _revertToDetecting() {
    this.currentProgram = null;
    this.confidence = 0;
    this._pendingMatch = null;
  ...
  /**
   * Schließt den aktuellen Zyklus ab: berechnet Dauer/Energie, führt
   * Post-hoc-Phasenanalyse durch, lernt ggf. das Profil und benachrichtigt.
   *
   * @param {object} eventData  – Snapshot vom CycleDetector bei Zyklusende
   */
  _onCycleFinished(eventData) {
    this.adapter.log.debug(
      `${this._name}: Zyklus wird abgeschlossen – Energie: ${(this.detector.accumulatedEnergy || 0).toFixed(1)} Wh`,
    );
    const trace = this.detector.getPowerTrace();
    const durationMs =
    ...
    // Post-hoc Phasenanalyse (Waschmaschine + Spülmaschine, nach Zyklusende)
    const devTypeF = (
      this.config.deviceType || "washing_machine"
    ).toLowerCase();
    if (
  ...
  _analyzePhasesPostHoc(trace, durationMs) {
    ...
    this.adapter.log.debug(
      `${this._name}: Post-hoc Phasen: ${phases.map((p) => `${p.phase}@${Math.round(p.tMs / 60000)}min`).join(", ")}`,
    );
    return phases;
  }
  ...
  _analyzePhasesPostHocDishwasher(trace, _durationMs) {
    ...
    if (mergedHeat.length === 0) {
      // Kein Heizblock erkannt (z.B. reines Kaltspülprogramm) –
      // gesamten Zyklus als Hauptspülgang werten
      addPhase("🔥 Hauptspülgang", startTs);
    } else {
      // Namen den Heizblöcken zuordnen:
      // 1 Block  → Hauptspülgang
      // 2 Blöcke → Hauptspülgang, Klarspülgang
      // 3+ Blöcke → Vorspülen, Hauptspülgang(e), Klarspülgang (letzter)
      let names;
      if (mergedHeat.length === 1) {
        names = ["🔥 Hauptspülgang"];
      } else if (mergedHeat.length === 2) {
        names = ["🔥 Hauptspülgang", "✨ Klarspülgang"];
      } else {
        names = ["🚿 Vorspülen"];
        for (let i = 1; i < mergedHeat.length - 1; i++) {
          names.push("🔥 Hauptspülgang");
        }
        names.push("✨ Klarspülgang");
      }

      // Deutliche Vorlaufzeit vor dem ersten Heizblock (>90s, z.B.
      // Wasser einlassen) noch der jeweils ersten Phase zuschlagen –
      // kein separates "Warten"-Label, um die Anzeige nicht unnötig
      // zu fragmentieren.
    ...
    // Trocknen: nach dem letzten Heizblock, sobald die Leistung
    // dauerhaft niedrig ist (kein weiterer Spülgang mehr folgt)
    if (mergedHeat.length > 0) {
      const lastHeat = mergedHeat[mergedHeat.length - 1];
      // Suche den Punkt nach dem letzten Heizblock, ab dem die
      // Leistung für den Rest des Zyklus durchgehend niedrig bleibt
      // (Restwärme-Trocknung / Kondensationstrocknung, <50W)
      let dryStart = null;
      for (let i = trace.length - 1; i >= 0; i--) {
      ...
      // Falls Leistung nach dem letzten Heizblock nie wieder ansteigt,
      // beginnt Trocknen direkt nach Heizende
      if (dryStart === null && lastHeat.end < endTs) {
        dryStart = lastHeat.end;
      }
      // Mindestens 2 Minuten Trocknen, sonst nicht separat ausweisen
      if (dryStart && endTs - dryStart >= 2 * 60000) {
        addPhase("💨 Trocknet", dryStart);
      }
    }

    this.adapter.log.debug(
      `${this._name}: Post-hoc Phasen (Spülmaschine): ${phases.map((p) => `${p.phase}@${Math.round(p.tMs / 60000)}min`).join(", ")}`,
    );
    return phases;
  }
    ...
    if (running2 && this.cycleStartTime) {
      ...
      if (devType === "dryer" || devType === "trockner") {
        ...
        const elapsedMin = this.cycleStartTime
          ...
          : 0;
        if (avg10 > 400) {
          detectedPhase = "♨️ Trocknet";
        } else if (avg30 > 300 && elapsedMin >= 5) {
          detectedPhase = "♨️ Trocknet";
        } else if (avg10 > 20 && elapsedMin >= 10) {
          detectedPhase = "❄️ Abkühlen";
        } else if (avg10 > 5 && elapsedMin >= 10) {
          detectedPhase = "👕 Anti-Knitter";
        } else {
          detectedPhase = "🔥 Aufheizen";
        }
      } else if (devType === "dishwasher" || devType === "geschirrspüler") {
        // Spülmaschine: gleiche Ordinal-Logik wie die Post-hoc-Analyse
        // (Heizblöcke zählen statt Momentanwert-Schwelle), angewandt
        // auf die bisherige Teilkurve. Kann sich noch ändern, sobald
        // ein weiterer Heizblock auftaucht (z.B. "Hauptspülgang" wird
        // rückwirkend zu "Vorspülen", falls doch noch ein 3. Heizblock
        // folgt) – unvermeidbar, solange der Zyklus nicht fertig ist,
        // aber deutlich sinnvoller als reines Momentanwert-Flackern.
        if (trace.length >= 3) {
