...
// Dryer anti-crease lock: how far a genuine anti-crease tumble spike pushes
// the lock's expiry forward, and the absolute ceiling (from when the lock
// was first set) that extensions can never exceed - real anti-crease
// duration varies cycle to cycle (load size, dampness), so a purely fixed
// lock length (learned duration + buffer) can run out while the dryer is
// still actually tumbling, letting the tail end be mistaken for a new cycle.
const ANTI_CREASE_LOCK_REFRESH_MS = 10 * 60 * 1000;
const ANTI_CREASE_LOCK_MAX_MS = 90 * 60 * 1000;
...
class WashDataManager {
  ...
  constructor(adapter, config, callbacks = {}) {
    ...
    this.detector = new CycleDetector(
      config,
      this._onDetectorState.bind(this),
      adapter,
    );
    ...
    this._dryerLockStartedAt = null; // when the lock was first set - for the absolute ceiling
  ...
  async stop() {
    ...
    if (this._progressResetTimer) {
      this.adapter.clearTimeout(this._progressResetTimer);
    }
    this.adapter.log.info(`${this._name} stopped`);
  }
  ...
  // ── Stuck power / heartbeat ──────────────────────────────────

  /**
   * Starts two things on the same 60s timer:
   *  1) stuck-HIGH detection (sensor stopped reporting while mid-cycle)
   *  2) a general heartbeat re-evaluation of the state machine.
   *
   * (2) matters because all state transitions (RUNNING → PAUSED → ENDING →
   * OFF) are driven by elapsed-time checks that only run *inside*
   * processReading() - and processReading() is normally only called when
   * the power sensor actually reports a new value. Many Shelly devices only
   * emit an update when the wattage changes beyond a small deadband, so
   * once a cycle finishes and power settles flat at ~0W, no further events
   * ever arrive and the adapter would otherwise wait forever for a "reading"
   * that never comes - leaving the cycle stuck as "running"/"ending"
   * indefinitely, with nothing to log because nothing actually failed.
   * Re-feeding the last known value once a minute lets the offDelay/
   * pauseDelay/minOffGap timers actually get evaluated and the cycle finish
   * normally even when the sensor goes quiet.
   */
  _startStuckPowerMonitor() {
    this._stuckTimer = this.adapter.setInterval(() => {
      ...
      // Only a genuinely stable HIGH reading (at/above the on-threshold,
      // i.e. plausibly mid-cycle) is worth treating as a possibly-stuck
      // sensor. A stable LOW reading (e.g. a few watts of standby power)
      // is normal, expected behavior, not evidence of a stuck sensor -
      // flagging it just causes repeated false alarms.
      const onThreshold = this.config.powerThreshold || 10;
      const stuckMs = Date.now() - this._lastPowerChangeTs;
      if (stuckMs > STUCK_POWER_MS && this._lastPowerVal >= onThreshold) {
        this.adapter.log.warn(
          `${this._name} stuck power: ${this._lastPowerVal}W for ${Math.round(stuckMs / 60000)} min`,
        );
        this.detector.processReading(0, Date.now());
        // Reset our own bookkeeping too - processReading() above calls
        // straight into the detector and bypasses _updateStuckPower(),
        // so without this the warning would keep re-firing every single
        // minute for the same episode instead of resolving once.
        this._lastPowerVal = 0;
        this._lastPowerChangeTs = Date.now();
        return;
      }

      // Heartbeat: re-evaluate the state machine with the last known value
      // even though the sensor hasn't sent anything new. Harmless when
      // nothing is due yet (processReading() just re-checks the elapsed
      // time and returns without a state change); necessary so a cycle
      // that ends on a flat, silent 0W reading can still transition
      // PAUSED → ENDING → OFF on schedule instead of waiting forever for
      // an event the sensor will never send.
      if (this._lastPowerVal !== null) {
        this.detector.processReading(this._lastPowerVal, Date.now());
      }
    }, 60 * 1000);
  }
  ...
  _stopStuckPowerMonitor() {
    if (this._stuckTimer) {
      this.adapter.clearInterval(this._stuckTimer);
      this._stuckTimer = null;
    }
  }
  ...
  processPowerReading(watts, timestamp) {
    ...
    if (antiKnitterActive && this._dryerLockUntil) {
      if (ts >= this._dryerLockUntil) {
        ...
        if (watts > realThreshold) {
          ...
          this._dryerHighStart = null;
          // A meaningful spike (motor tumble, not just idle standby noise)
          // is itself evidence the dryer is still actually doing its
          // anti-crease thing - push the lock's expiry forward instead of
          // letting a fixed-length lock run out mid-tumble, capped at an
          // absolute ceiling from when the lock was first set.
          if (watts > 10 && this._dryerLockStartedAt) {
            const extended = Math.min(
              ts + ANTI_CREASE_LOCK_REFRESH_MS,
              this._dryerLockStartedAt + ANTI_CREASE_LOCK_MAX_MS,
            );
            if (extended > this._dryerLockUntil) {
              this.adapter.log.debug(
                `${this._name}: anti-crease still active (${watts.toFixed(0)}W) – lock extended to ${Math.round((extended - ts) / 60000)} min from now`,
              );
              this._dryerLockUntil = extended;
            }
          }
          this.adapter.log.debug(
    ...
    if (this.currentState === STATES.STARTING) {
      this.adapter.log.debug(
        `${this._name}: STARTING – ${watts.toFixed(0)}W (threshold: ${this.config.startEnergyThreshold || 10}W)`,
      );
    }
  ...
  _runMatching() {
    ...
    if (result) {
      ...
      // Locking itself now happens unconditionally in _setProgram() as
      // soon as a program is confirmed at all - see the comment there for
      // why (a confirmation via score accumulation can land anywhere from
      // 60% up, well below what used to be required here to lock).
  ...
  _setProgram(profileId, name, confidence) {
    ...
    // Lock immediately on every confirmation, not just if some later,
    // individual reading happens to hit LOCK_CONFIDENCE (75%) on its own -
    // a program confirmed via score accumulation can land anywhere from
    // MIN_CONFIDENCE_FOR_SET (60%) up, and without this, a confirmation in
    // the 60-74% range stayed unlocked and could later be wiped back to
    // "detecting..." by a run of unmatched readings (_revertToDetecting())
    // - typically during a long, uniform late-cycle phase where
    // correlation naturally weakens, even though the early-cycle match
    // that confirmed it in the first place is normally far more reliable
    // (found via a real case: correctly detected early, reverted to
    // "detecting..." near the end). The high-confidence override path
    // (OVERRIDE_CONFIDENCE, OVERRIDE_PERSIST rounds) remains the only way
    // to change a locked program afterward.
  ...
  _updateTimeEstimate(now) {
    // Also calculate progress/remaining time for bestCandidate
    const activeProgram =
      this.currentProgram ||
    ...
    // Phase-based estimate: "time left in the current phase, plus the
    // typical duration of every phase that historically comes after it for
    // this profile" (learned per-phase from past cycles' phaseHistory, see
    // learnFromCycle()). Far more accurate than a single whole-cycle
    // average once a cycle is well underway, especially for a phase (like
    // a dryer's main drying phase) whose position relative to the total
    // cycle length varies a lot between runs. Only trusted once at least 3
    // past cycles have data for the CURRENT phase specifically, and once
    // we actually know when that phase started (_phaseHistory's last entry).
    let phaseBasedRemainingMs = null;
    if (
      profile.phaseStats &&
      this._stablePhase &&
      this._phaseHistory &&
      ...
      const seq = profile.phaseStats.sequence;
      const idx = seq.indexOf(this._stablePhase);
      const currentPhaseSamples =
        profile.phaseStats.durations[this._stablePhase];
      if (
        idx !== -1 &&
        currentPhaseSamples &&
        currentPhaseSamples.length >= 3
      ) {
        const currentPhaseStartTs =
          this._phaseHistory[this._phaseHistory.length - 1].ts;
        const elapsedInPhaseMs = now - currentPhaseStartTs;
        const typicalPhaseMs = profile.phaseStats.durationMs[this._stablePhase];
        const remainingInPhaseMs = Math.max(
          0,
          typicalPhaseMs - elapsedInPhaseMs,
        );
        let futurePhasesMs = 0;
        for (let i = idx + 1; i < seq.length; i++) {
          futurePhasesMs += profile.phaseStats.durationMs[seq[i]] || 0;
        }
        phaseBasedRemainingMs = remainingInPhaseMs + futurePhasesMs;
      }
    }

    // ── Phase safety net ──────────────────────────────────────────
    // If the currently detected phase is one of these short, predictable
    // terminal phases but the estimate still shows a lot of remaining
    // time, that's a sign the estimate is too high - clamp it. Spinning is
    // the washer's short final phase; cooling is the dryer's equivalent
    // (its main "drying" phase - the one whose real duration varies the
    // most, see the CV-based blend weight below - is deliberately NOT
    // capped here, since unlike these two it has no short, predictable
    // upper bound; the phase-based estimate above handles that phase
    // instead, once enough data has been learned for it).
    const PHASE_MAX_REMAINING_MS = {
      spinning: 25 * 60000, // rarely lasts >25min
      cooling: 15 * 60000, // dryer cool-down, rarely lasts >15min
    };
    let phaseAdjustedRemainingMs = null;
    if (PHASE_MAX_REMAINING_MS[this._stablePhase] !== undefined) {
      phaseAdjustedRemainingMs = PHASE_MAX_REMAINING_MS[this._stablePhase];
    }

    // Combination: once a trustworthy phase-based estimate is available, it
    // becomes the primary signal (blended lightly with the live
    // energy-based one for extra robustness), replacing the plain
    // time-based estimate - it directly reflects "which phase is this
    // cycle actually in right now", which a single whole-cycle average
    // cannot. Otherwise, fall back to the weighted time/energy blend from
    // before (weighted by durationCV - see learnFromCycle()). Either way,
    // still capped from above by the phase safety net.
    let blendedRemainingMs;
    if (phaseBasedRemainingMs !== null) {
      blendedRemainingMs =
        energyBasedRemainingMs !== null
          ? phaseBasedRemainingMs * 0.7 + energyBasedRemainingMs * 0.3
          : phaseBasedRemainingMs;
    } else {
      blendedRemainingMs = timeBasedRemainingMs;
      if (energyBasedRemainingMs !== null) {
        const cv = profile.durationCV || 0;
        const energyWeight = Math.min(0.8, Math.max(0.3, 0.3 + cv * 1.25));
        blendedRemainingMs =
          timeBasedRemainingMs * (1 - energyWeight) +
          energyBasedRemainingMs * energyWeight;
      }
    }
    ...
    // Recompute progress from the SAME blended remaining-time estimate,
    // rather than the plain elapsed/historical-average ratio computed
    // above - otherwise "Fortschritt: X%" and "Fertig um HH:MM" could tell
    // two inconsistent stories (e.g. progress jumping ahead while the
    // predicted finish time barely moves, or vice versa), since they'd be
    // driven by two different models. Capped at 99% while still running -
    // only the actual cycle end reports 100%.
    const blendedProgressPct =
      elapsedMs + remainingMs > 0
        ? Math.min(
            99,
            Math.round((elapsedMs / (elapsedMs + remainingMs)) * 100),
          )
        : progressPct;
  ...
  _onCycleFinished(eventData) {
    ...
    // _phaseHistory is only ever lazily initialized, never cleared between
    // cycles - without this reset, entries from every past cycle since
    // adapter start keep accumulating and get re-timestamped relative to
    // the *current* cycleStartTime above, producing wildly wrong (often
    // large negative) tMs values for anything but the most recent phases.
    this._phaseHistory = [];
    ...
    // Progress reset after 5min
    if (typeof this.callbacks.onTimeUpdate === "function") {
      this.callbacks.onTimeUpdate(0, Math.round(durationMs / 1000), 100);
    }
    this._progressResetTimer = this.adapter.setTimeout(() => {
      if (typeof this.callbacks.onTimeUpdate === "function") {
    ...
    // Save lastCycleCompleted BEFORE detector.reset() clears it
    ...
    this.adapter.log.info(
      `${this._name} ${cycleStatus}: ${cycle.matchedProfile}, ` +
        `${Math.round(durationMs / 60000)} min, ${cycle.energyWh.toFixed(2)} Wh, ${trace.length} points`,
    );

    this.adapter.log.debug(
      `${this._name}: cycle saved – ${cycle.matchedProfile}, ${Math.round(cycle.durationMs / 60000)} min, ${cycle.energyWh.toFixed(2)} Wh`,
    );
  ...
  // ── Post-hoc phase analysis ──────────────────────────────────

  /**
   * Analyzes the complete power curve of a finished washing machine cycle
   * after the fact, and derives named phases from it (heating/washing/
   * rinsing/spinning/etc.).
   *
   * @param {Array<{ts:number, watts:number}>} trace  – complete power curve
   * @param {number} durationMs  – total cycle duration
   * @returns {Array<{phase:string, tMs:number}>}  – detected phases with start time
   */
  _analyzePhasesPostHoc(trace, durationMs) {
    ...
    // Merge heating phases that are close together (<60s apart)
    const mergedHeat = [];
    ...
    // Spin = rising block >250W in the last 15% of the cycle
    let spinStart = null,
      spinEnd = endTs;
    ...
    // Minimum spin duration: 2 minutes
    if (spinStart && spinEnd - spinStart < 2 * 60000) {
      this.adapter.log.debug(
        `${this._name}: spin discarded - too short (${Math.round((spinEnd - spinStart) / 60000)}min)`,
      );
      spinStart = null;
    }
    if (spinStart) {
      this.adapter.log.debug(
        `${this._name}: spin detected from ${Math.round((spinStart - startTs) / 60000)}min`,
      );
    }
  ...
  // ── Post-hoc phase analysis: dishwasher ────────────────────
  // Order per the actual program sequence:
  //   pre-wash (cold, coarse soil) → main wash (hot, cleaning) →
  //   final rinse (hot, rinsing) → drying (residual heat/condensation)
  //
  // A dishwasher heats up again for each of the above wash passes - so a
  // plain instantaneous-value threshold (>1200W = "heating") can't
  // distinguish between the passes. Instead, contiguous heating blocks
  // are counted and named by their position in the cycle.
  /**
   * Analogous to _analyzePhasesPostHoc, but for dishwashers: counts
   * contiguous heating blocks and names them ordinally (pre-wash /
   * main wash(es) / final rinse / drying).
   *
   * @param {Array<{ts:number, watts:number}>} trace  – complete power curve
   * @param {number} _durationMs  – total duration (currently unused, kept for signature symmetry)
   * @returns {Array<{phase:string, tMs:number}>}  – detected phases with start time
   */
  _analyzePhasesPostHocDishwasher(trace, _durationMs) {
    ...
    const HEAT_W = 1000; // W – heating element (heating water/drying)

    // ── 1. Find heating blocks (same as washing machine) ─────
    const heatSegs = [];
    let inHeat = false,
      heatStart = null;
    for (let i = 0; i < trace.length; i++) {
    ...
    // Merge heating blocks that are close together (<90s apart, e.g. the
    // heating element cycling on/off during a wash-pass heat-up)
    const mergedHeat = [];
    for (const seg of heatSegs) {
      const last = mergedHeat[mergedHeat.length - 1];
  ...
  /** Computes configuration suggestions (e.g. thresholds) from the cycle history so far. */
  /**
   * Device-type-appropriate offDelayMin default, mirroring the fallback
   * logic in main.js's device config construction (kept in sync there).
   *
   * @returns {number} – default off-delay in minutes for the current device type
   */
  _defaultOffDelayMin() {
    const dt = (this.config.deviceType || "").toLowerCase();
    ...
    if (dt === "dishwasher" || dt === "geschirrspüler") {
      return 8;
    }
    return 5;
  }
  ...
  _computeSuggestedSettings() {
    ...
    // Start energy: 1% of average consumption, min 0.5Wh, max 10Wh
    // Logic: cycle starts once 1% of the typical energy has been consumed
    ...
    // Power-on threshold: 5% of maximum consumption, min 5W, max 100W
    ...
    // Off delay: estimate from the cycle-end pattern (min 2, max 15)
    ...
    // Only suggest fields that deviate >10% from the current config
    ...
    if (
      diff(suggestedOffDelay, cfg.offDelayMin || this._defaultOffDelayMin())
    ) {
      result.offDelayMin = suggestedOffDelay;
    }
    this.adapter.log.debug(
      `${this._name}: suggestion check offDelayMin – suggested=${suggestedOffDelay}, current cfg.offDelayMin=${cfg.offDelayMin}, default=${this._defaultOffDelayMin()}, dismissed=${this.profileStore.getDismissedSuggestions().offDelayMin}`,
    );

    // Remove any field whose suggested value exactly matches what the
    // user already dismissed for that field - a different (new) suggested
    // value for the same field is NOT filtered, so it reappears if the
    // recommendation genuinely changes based on more recent cycles.
    const dismissed = this.profileStore.getDismissedSuggestions();
    for (const field of Object.keys(dismissed)) {
      if (result[field] !== undefined && result[field] === dismissed[field]) {
        delete result[field];
      }
    }

    // Only show if there are actually recommendations
    const hasRecommendations = Object.keys(result).length > 2; // more than basedOnCycles + computedAt
    this._suggestedSettings = hasRecommendations ? result : null;
  }

  /** @returns {object|null} – most recently computed configuration suggestions, or null */
  getSuggestedSettings() {
    return this._suggestedSettings;
  }

  /**
   * Dismisses a single suggested-settings field so it stops being shown,
   * unless the underlying recommendation later changes to a different value.
   *
   * @param {string} field  – suggestion field key (e.g. 'offDelayMin')
   * @param {number} value  – the specific suggested value to dismiss
   */
  async dismissSuggestion(field, value) {
    await this.profileStore.dismissSuggestion(field, value);
    // Re-filter immediately so the caller sees the updated list right away
    if (this._suggestedSettings && this._suggestedSettings[field] === value) {
      delete this._suggestedSettings[field];
      const hasRecommendations =
        Object.keys(this._suggestedSettings).length > 2;
  ...
  /** @param {{maxWatts:number, durationMs:number}} ak  – new anti-crease reference values */
  setAntiKnitterConfig(ak) {
    this._antiKnitter = ak;
    this.adapter.log.info(
      `${this._name}: anti-crease configuration set: ${Math.round(ak.durationMs / 60000)} min, max ${Math.round(ak.maxWatts)}W`,
    );
  }

  /** Deletes the anti-crease reference and immediately lifts any active lock period. */
  clearAntiKnitterConfig() {
    this._antiKnitter = null;
    // Also immediately lift any currently active lock period, so the
    // next cycle can be recorded without interference.
    this._dryerLockUntil = null;
    this._dryerLockStartedAt = null;
    this._dryerDropTriggered = false;
    this._dryerHighStart = null;
    this.adapter.log.info(`${this._name}: anti-crease pattern deleted`);
  }

  // ── Public API ───────────────────────────────────────────────

  /** @param {string} cycleId  – cycle to look up */
  getTrace(cycleId) {
    return this.traceStore.getTrace(cycleId);
  }

  /**
   * @param {string} cycleId  – cycle to trim
   * @param {number} s  – new start time (Unix ms)
   * @param {number} e  – new end time (Unix ms)
   */
  ...
  /**
   * @param {string} cycleId  – cycle to split
  ...
  /** @param {string} name  – name for the new manual profile, based on the last cycle */
  createProfileFromLastCycle(name) {
    if (this.cycleHistory.length === 0) {
      throw new Error("No cycle available");
    }
  ...
  /** @returns {object} – current status (live display) for this device */
  getStatus() {
    return this._buildStatus();
  }

  /** @returns {Array<object>} – cycle history for this device */
  getCycleHistory() {
  ...
  /** @returns {object} – complete status snapshot for the admin UI/data points */
  _buildStatus() {
    const elapsedTime = this.cycleStartTime
      ? Math.round((Date.now() - this.cycleStartTime) / 60000)
      : 0;
    // Calculate progress - also for bestCandidate (from 50% confidence)
    let cycleProgress = 0;
    const progSource =
      this.currentProgram ||
      (this._bestCandidate && this._bestCandidate.confidence >= 0.5
    ...
    // Phase detection from power history – device-type specific
    let phase = "";
    const running2 =
      this.currentState === STATES.RUNNING ||
      this.currentState === STATES.PAUSED;
    ...
    if (running2 && this.cycleStartTime) {
      ...
      // Longer average: last 30 points (~5min) for trend analysis
      ...
      // Record phase changes for the graph (min. 90s per phase)
