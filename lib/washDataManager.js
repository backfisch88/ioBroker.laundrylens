...
class WashDataManager {
  ...
  _updateTimeEstimate(now) {
    ...
    // estimates, additionally capped from above by the phase safety net.
    // The energy-based estimate is weighted more heavily for profiles
    // whose real duration is known (from durationCV, see learnFromCycle())
    // to vary a lot cycle to cycle - a dryer's drying time depends heavily
    // on load size/dampness, so its historical time average alone is a
    // poor predictor for any one specific run, while the live energy pace
    // is a much better real-time signal for those. A consistent program
    // (low CV) keeps leaning mostly on the proven time-based estimate.
