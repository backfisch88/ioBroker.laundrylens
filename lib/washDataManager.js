...
class WashDataManager {
    ...
    processPowerReading(watts, timestamp) {
        ...
        const antiKnitterEnabled = (devType2 === 'dryer' || devType2 === 'trockner') && this.config.ignoreAntiKnitter === false;
        ...
        if (!antiKnitterActive && this._dryerLockUntil) {
            ...
            this.adapter.log.debug(`${this._name}: Anti-Knitter inaktiv (deaktiviert oder kein Muster) – bestehende Sperrzeit verworfen`);
            this._dryerLockUntil = null;
            this._dryerDropTriggered = false;
            this._dryerHighStart = null;
        }
    ...
    _onCycleFinished(eventData) {
        ...
        } else if ((devTypeF === 'dishwasher' || devTypeF === 'geschirrspüler') && trace.length >= 5) {
            cycle.phaseHistory = this._analyzePhasesPostHocDishwasher(trace, durationMs);
    ...
    _analyzePhasesPostHocDishwasher(trace, _durationMs) {
        ...
        if (mergedHeat.length === 0) {
            ...
            if (mergedHeat.length === 1) {
                ...
                names = ['🚿 Vorspülen'];
                for (let i = 1; i < mergedHeat.length - 1; i++) names.push('🔥 Hauptspülgang');
        ...
        if (mergedHeat.length > 0) {
            ...
            let dryStart = null;
            for (let i = trace.length - 1; i >= 0; i--) {
                if (trace[i].ts <= lastHeat.end) break;
                if (avg(i) >= 50) { dryStart = trace[i + 1] ? trace[i + 1].ts : trace[i].ts; break; }
            }
            // Falls Leistung nach dem letzten Heizblock nie wieder ansteigt,
            // beginnt Trocknen direkt nach Heizende
            if (dryStart === null && lastHeat.end < endTs) dryStart = lastHeat.end;
        ...
        this.adapter.log.debug(`${this._name}: Post-hoc Phasen (Spülmaschine): ${phases.map(p => `${p.phase}@${Math.round(p.tMs/60000)}min`).join(', ')}`);
        return phases;
    }
        ...
        if (running2 && this.cycleStartTime) {
            ...
            if (devType === 'dryer' || devType === 'trockner') {
                ...
                if (trace.length >= 3) {
                    ...
                    const livePhases = this._analyzePhasesPostHocDishwasher(trace, elapsedMsSoFar);
                    if (livePhases.length > 0) {
                        detectedPhase = livePhases[livePhases.length - 1].phase;
                    }
                }
                if (!detectedPhase) detectedPhase = '🚿 Vorspülen';
