...
class WashDataManager {
    ...
    processPowerReading(watts, timestamp) {
        ...
        if (!antiKnitterEnabled && this._dryerLockUntil) {
            // Funktion wurde deaktiviert (oder Gerätetyp geändert) – alte Sperre verwerfen
            this.adapter.log.debug(`${this._name}: Anti-Knitter deaktiviert – bestehende Sperrzeit verworfen`);
            this._dryerLockUntil = null;
            this._dryerDropTriggered = false;
            this._dryerHighStart = null;
        }
    ...
    _analyzePhasesPostHocDishwasher(trace, durationMs) {
        if (!trace || trace.length < 3) return [];
        ...
        const NAMES = ['🚿 Vorspülen', '🔥 Hauptspülgang', '✨ Klarspülgang'];
